        ...
        <div v-if="count > 0 && reFresh">
            <div class="reply-item" v-for="(comment, index) of commentList" :key="comment.id">
                ...
                <div class="content-warp">
                    ...
                    <div class="sub-reply-item" v-for="reply of comment.childList" :key="reply.id">
                    ...
                    <div ref="readMoreRef" class="view-more" v-show="comment.childCount > 3">
                        <span>共{{ comment.childCount }}条回复, </span>
                        <span class="view-more-btn" @click="readMoreComment(index, comment)">点击查看</span>
                    </div>
                    <!-- 分页的页数显示 -->
                    <Paging ref="pageRef" :total="comment.childCount" :index="index" :commentId="comment.id"
...
const readMoreComment = (index: number, comment: Comment) => {
    getReplyList(comment.id, { pageNum: 1, pageSize: 5 }).then(({ data }) => {
        comment.childList = data.data;
        // 回复大于5条展示分页
        if (comment.childCount > 5) {
...
const getCurrentPage = (current: number, index: number, commentId: number) => {
    getReplyList(commentId, { pageNum: current, pageSize: 5 }).then(({ data }) => {
        commentList.value[index].childList = data.data;
    });
};
// 点击回复的数据
...
const getList = () => {
    getCommentList(queryParams.value).then(({ data }) => {
        if (queryParams.value.pageNum == 1) {
            commentList.value = data.data.rows;
        } else {
            commentList.value.push(...data.data.rows);
        }
...
const reloadReplies = (index: number) => {
    getReplyList(commentList.value[index].id, {
        pageNum: pageRef.value[index].current,
        pageSize: 5,
    }).then(({ data }) => {
        commentList.value[index].childList = data.data;
        commentList.value[index].childCount++;
        // 隐藏回复框
        replyRef.value[index].setReply(false);
        // 回复大于5条展示分页
        if (commentList.value[index].childCount > 5) {
            pageRef.value[index].setPaging(true);
        }
        // 隐藏查看更多
        readMoreRef.value[index].style.display = "none";
